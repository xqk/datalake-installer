...
if [ -f /usr/bin/dectl ]; then
   # 获取已安装的 DataLake 的运行目录
   DE_BASE=`grep "^DE_BASE=" /usr/bin/dectl | cut -d'=' -f2`
   dectl uninstall
fi

set -a
if [[ $DE_BASE ]] && [[ -f $DE_BASE/datalake/.env ]]; then
...
datalake_conf=${conf_folder}/datalake.properties
...
echo -e " 当前部署模式为 ${DE_ENGINE_MODE}，如需切换模式，\n 请修改 $DE_BASE/datalake/.env 中的 DE_ENGINE_MODE 变量后，\n 重新执行 bash install.sh 即可\n" 2>&1 | tee -a ${CURRENT_DIR}/install.log
echo -e "*******************************************************\n" 2>&1 | tee -a ${CURRENT_DIR}/install.log


if [[ -f $datalake_conf ]] && [[ ! ${DE_EXTERNAL_DORIS} ]]; then
   export DE_DORIS_DB=$(prop $datalake_conf doris.db)
   export DE_DORIS_USER=$(prop $datalake_conf doris.user)
   export DE_DORIS_PASSWORD=$(prop $datalake_conf doris.password)
   export DE_DORIS_HOST=$(prop $datalake_conf doris.host)
   export DE_DORIS_PORT=$(prop $datalake_conf doris.port)
   export DE_DORIS_HTTPPORT=$(prop $datalake_conf doris.httpPort)
...
if [[ -f $datalake_conf ]] && [[ ! ${DE_EXTERNAL_KETTLE} ]]; then
   export DE_CARTE_HOST=$(prop $datalake_conf carte.host)
   export DE_CARTE_PORT=$(prop $datalake_conf carte.port)
   export DE_CARTE_USER=$(prop $datalake_conf carte.user)
   export DE_CARTE_PASSWORD=$(prop $datalake_conf carte.passwd)
...
log "根据安装配置参数调整配置文件"
cd ${templates_folder}
templates_files=( datalake.properties mysql.env )
...
log "配置 datalake Service"
cp ${DE_RUN_BASE}/bin/datalake/datalake.service /etc/init.d/datalake
chmod a+x /etc/init.d/datalake
if which chkconfig;then
   chkconfig --add datalake
fi

if [ -f /etc/rc.d/rc.local ];then
   datalakeService=`grep "service datalake start" /etc/rc.d/rc.local | wc -l`
   if [ "$datalakeService" -eq 0 ]; then
      echo "sleep 10" >> /etc/rc.d/rc.local
      echo "service datalake start" >> /etc/rc.d/rc.local
   fi
...
echo -e "======================= 安装完成 =======================\n" 2>&1 | tee -a ${CURRENT_DIR}/install.log
echo -e "请通过以下方式访问:\n URL: http://\$LOCAL_IP:$DE_PORT\n 用户名: admin\n 初始密码: datalake" 2>&1 | tee -a ${CURRENT_DIR}/install.log
